...
uses
  Generics.Defaults, Generics.Collections, Contest, Station, DxStn, Log, GetDataPath;
...
type
  ...
  TPendingCall = record
    ...
    class function comparePendingCall(constref left, right : TPendingCall) : integer; static;
  end;
...
uses
  ...
  Ini,
  Math,
{$ifdef DISTRIBUTION_REPORT}      // for ShowMessage
...
class function TPendingCall.comparePendingCall(constref left, right : TPendingCall) : integer;
begin
...
function ComparePendingCallWrapper(constref left, right: TPendingCall): integer;
begin
...
begin
  ...
  PendingCallComparer := TComparer<TPendingCall>.Construct(ComparePendingCallWrapper);
end;
...
begin
  ...
  try
    ...
    slst.LoadFromFile(GetDataPath.GetDataPath + 'FDGOTA.TXT');
...
begin
  ...
  try
    ...
    begin
      Len := (item.Value div CallsPerDot);
      Str := StringOfChar('*', Len);
