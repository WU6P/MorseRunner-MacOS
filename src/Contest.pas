...
begin
  Log.CallSent := (Globals.GEdit1Text <> '') and
    (Globals.GEdit1Text = Self.Me.HisCall);
end;
...
begin
  if not ValidateExchField1(AExch1) then
    AExchError := format('Missing/Invalid %s',
      [Exchange1Settings[Globals.GRecvExchTypes.Exch1].C])
  else if not ValidateExchField2(AExch2) then
    AExchError := format('Missing/Invalid %s',
      [Exchange2Settings[Globals.GRecvExchTypes.Exch2].C])
  else
    AExchError := '';
...
begin
  ...
  if Qso.Exch2.IsEmpty and (Globals.GRecvExchTypes.Exch2 <> etNaQpNonNaExch2) then
    Qso.Exch2 := '?';
end;
...
begin
  ...
  if Me.State = stSending then
    ...
    Blk := Me.GetBlock;
    //self-mon. gain
    Smg := Power(10, (Globals.GVolumeSliderValue - 1) * 3);

    // apply linear rolloff towards zero between -57 and -60db (i.e. Smg=0 @ -60db)
    if Globals.GVolumeSliderValue < 0.05 then       // @ -57db, value = 3/60 = 0.05
      Smg := Smg * Globals.GVolumeSliderValue * 60; // 60 dB steps in [-60,0dB]
...
{$ifdef DEBUG}
     if Globals.BDebugExchSettings then
     begin
...
{$endif}
  ...
  if (BlocksToSeconds(BlockNumber) >= (Duration * 60)) or FStopPressed then
    ...
    if RunMode = rmHst then
      begin
      if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc();
      FStopPressed := false;
      if Assigned(Globals.GPopupScoreHstProc) then Globals.GPopupScoreHstProc();
      end
      ...
      if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc();
      FStopPressed := false;
      if Assigned(Globals.GPopupScoreWpxProc) then Globals.GPopupScoreWpxProc();
      end
    else
      begin
      if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc();
